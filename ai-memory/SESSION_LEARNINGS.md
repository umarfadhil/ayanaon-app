...
## v2.6.3 release alignment (2026-08-19) — parking/Travel Mode branch
...
- **Superseded by the merge note below**: this entry was written on a branch that diverged from the modifier-groups branch below at v2.6.2 — both independently picked "v2.6.3" as the next version and were reconciled in one merge commit rather than renumbered, since this commit was already pushed to `origin/cloudflare-migration` before the conflict was discovered.

## Modifier Groups ("Varian Bertingkat") storefront support (2026-08-25)
- AyaKasir petalytix v1.3.54 added tenant-level Modifier Groups (option_groups/option_group_values/product_option_group_links, orthogonal to `variants`) — orthogonal per-product option sets (e.g. Level Gula) the cashier picks per cart line. Found by walking petalytix release commits (97d782a); not yet in either repo's ai-memory when this session started.
- Wired end-to-end: `petalytix/src/lib/ayanaon-partner.ts` `fetchMenuHighlights()` now fetches linked ACTIVE groups+values per product (only when `tenant.modifier_groups_enabled`) and pushes `modifierGroups: [{name, values:[{name, priceAdjustment}]}]` per menu item, alongside `variants`.
- `netlify/functions/api.js`: new `cleanMerchantModifierGroups`/`cleanMerchantModifierGroupValues` sanitizers (caps 10 groups/30 values, drops empty groups); `modifierGroups` is product-level so it applies to a bare item row AND every variant row of that product identically (never per-variant).
- `/toko/:slug`: a modifier-bearing row's `<li>` carries `data-modifiers` (JSON) and swaps its +/- stepper for a single "+" (`data-open-modifier`) that opens a shared `#toko-modifier-modal` (emitted only when `hasAnyModifierGroups`); confirming adds a cart line keyed `dataName|group:value;...` so two different combinations of the same product/variant never merge (mirrors AyaKasir POS's own `lineKey` fix, SESSION_LEARNINGS 2026-08-24 in petalytix).
- The WA-cart engine (`waScript`) was rewritten from DOM-derived state (one `.toko-menu__count` per row = the cart) to an explicit in-memory `cart` array of `{key,name,price,qty}` lines, because one row can now back several distinct cart lines. `availScript` (separate script tag) can't see that closure, so it zeroes a sold-out row's lines via a new `window.tokoZeroItem(baseName)` hook instead of writing to `.toko-menu__count` directly.
- v1 price_adjustment is always 0 upstream, but both sanitizers and the picker UI support it anyway (real DB column already exists in petalytix, not hypothetical) — shows "+RpX" only when non-zero.
- Verified: `node --check`, full `npm run test:deployment` (40/40 incl. 3 new modifier tests in `tests/merchant-visibility.test.js`), and a scratch smoke render confirming the modal only appears for merchants that actually have a modifier-bearing item. Petalytix side typechecked via a scoped `tsconfig.checkayanaonmodifiers.json` (gitignored, same convention as `tsconfig.checkfinance.json`).
- Not done: Android POS has no modifier UI yet (petalytix confirmed this); AyaKasir order-page (`/order/:token`, the Pesanan Online QR flow) does not render modifierGroups either — only the `/toko` storefront was in scope for this session.

## Modifier Groups not appearing on live /toko — root cause + local verification (2026-08-25, same day)
- Owner reported Jus Mangga's "Es Batu" modifier missing on live `/toko/cafe-demo-kota-bogor`. Root cause: BOTH repos' changes above were still uncommitted/undeployed (confirmed via `git status`) — the live site never had the new code, and the last successful push for that tenant (`ayanaon_synced_at`) predates it, so the stored merchant doc had no `modifierGroups` at all. Data itself was already correct: tenant `modifier_groups_enabled=true`, Jus Mangga actively linked to an active "Es Batu" group with 3 active values.
- **Real gap found while tracing this, independent of the deploy issue**: `supabase/migration_ayanaon_auto_sync.sql`'s trigger only watches `products`/`variants`/`categories`/`tenants` — NOT `product_option_group_links`/`option_groups`/`option_group_values`, nor `tenants.modifier_groups_enabled`. So even once deployed, a modifier-only edit will never flip `ayanaon_pending_sync` and the 5-minute cron will never pick it up — needs a follow-up migration extending the trigger set (not applied yet; owner chose to defer deploy).
- Verified the fix works with **zero deploys**, per owner's request to see it in local dev first: ran `wrangler dev --env local` (shares prod Mongo per existing convention — this is a real write, not a sandbox), then ran the ACTUAL `pushAyanaonListing()` from `petalytix/src/lib/ayanaon-partner.ts` via `npx tsx` (no Next.js-specific imports in its dependency chain, so it runs standalone) with `AYANAON_PARTNER_BASE_URL` pointed at the local Worker — this pushed real Supabase data through the real petalytix code into the locally-running ayanaon-app's sanitizer/storage.
- Then drove the actual UI with Playwright (`npx playwright install chromium` + local `playwright` package in the scratchpad, no project skill existed for this repo yet): opened `/toko/cafe-demo-kota-bogor`, clicked Jus Mangga's "+", confirmed the "Pilih Modifier" modal showed group "Es Batu" with Less/Normal/Extra, selected Extra + qty 2, confirmed the cart showed "Jus Mangga (Extra)" Rp8.300 x 2 = Rp16.600, zero console errors. Screenshots confirm this visually. `.toko-menu__acc` (ACCORDION layout) details must be force-opened (`el.open = true`) before Playwright can interact with rows in later categories — collapsed `<details>` content is DOM-attached but not "visible".
- Because the local push already wrote the new `modifierGroups` data into the SAME production merchant document (local dev has no separate DB), once ayanaon-app is actually deployed with this session's code, Cafe (Demo) will already render correctly with no further sync needed — only tenants that get a NEW modifier edit after deploy will hit the auto-sync trigger gap above.

## Modifier row was missing its "-" button (2026-08-25, later)
- A modifier-bearing row originally shipped with only a single "+" (opens the picker) and no "-", since one row can back several distinct cart lines and a bare "-" has no single obvious target. Owner correctly flagged this as inconsistent with every other row's +/- pair, screenshotted from their own local run.
- Fix: added the "-" back (`data-modifier-minus`), backed by `lastKeyByBaseName[baseName]` — set every time `confirmModifierSelection()` adds/updates a line — so "-" decrements the combination just confirmed, matching a plain row's one-tap feel. If that exact line was since removed (e.g. via the cart modal) or never set, `decrementModifierRow()` falls back to whichever combination for that row still has qty > 0, so the button is never a silent no-op as long as something is in the cart for it.
- Verified via the same local dev + Playwright loop (server was already running, hot-reloaded on save): adding Normal x1 then one "-" click correctly zeroes it and disables the cart button; adding Less x1 + Extra x1 (two different combos, row badge aggregates to 2) then one "-" click correctly removes only the most-recently-added "Extra" line, leaving "Jus Mangga (Less)" in the cart. Zero console errors both times.

## Merchant map marker: category-adaptive logo-fallback icon (2026-08-25)
- Owner follow-up: a merchant with no `logoUrl` always showed a generic 🏪 on the map marker + pin-list row — not tied to the tenant's actual business type.
- Initial pass wrongly assumed `merchant.category` was unconstrained free text and used only keyword substring guessing. Owner corrected: it should key off the ACTUAL store category. Confirmed `category` is in practice constrained to a fixed 57-entry enum — `AYANAON_CATEGORIES` in the **petalytix** repo (sibling dir, `src/lib/ayanaon-categories.ts`, mirrored byte-identical in the Android app's `BusinessCategories.kt`) — rendered as a `<select>` dropdown in `AyanaonStoreSettings.tsx`; ayanaon's own `sanitizeMerchantPayload` (`netlify/functions/api.js` ~6006) just stores whatever string arrives (≤60 chars, defaults `'Restoran'`) with no server-side enum validation, which is why this wasn't obvious from this repo alone.
- Rebuilt `getMerchantCategoryIcon(category)` in `public/app.js` (~3159): **primary** = exact (trim+lowercase) match against a hand-built `MERCHANT_CATEGORY_ICONS` map covering all 57 `AYANAON_CATEGORIES` values (e.g. `'toko kelontong'`→🏪, `'bengkel motor'`→🏍️, `'klinik hewan'`→🐾, `'lainnya'`→🏪); **secondary** = literal emoji already embedded in the string; **tertiary** = the original ~25-group keyword substring rules, kept only as a net for legacy docs predating the picker (e.g. old hardcoded `'Kuliner'` default) or any off-enum value; final fallback stays 🏪.
- If petalytix's `AYANAON_CATEGORIES` list changes, `MERCHANT_CATEGORY_ICONS` in app.js must be updated to match — there's no shared source of truth between the two repos for this list, just this note.
- Wired into `createMerchantMarkerElement` (map marker, app.js ~3221) and `updatePinListPanel`'s "Toko / UMKM" list rows (app.js ~4742); `buildMerchantPopupNode` (InfoWindow) and `/toko/:slug` SSR (`.toko-logo` in api.js) both just omit the image when logo-less rather than showing 🏪, so left unchanged.

## v2.6.3 release alignment (2026-08-25)
- v2.6.3 packages the August 25 `/toko` Modifier Groups feature (picker modal, in-memory WA cart engine, "-" button parity) and the category-matched merchant fallback icon — all four entries above were still uncommitted when this release pass started.
- Release metadata advanced together: root package + lockfile version (2.6.2 → 2.6.3), README release notes, `PROJECT_OVERVIEW.md` current-version line and the `merchants` collection description (now names `variants[]`/`modifierGroups[]`), and the service-worker cache revision. `CODE_RULES.md`/`FILE_MAP.md` were already updated earlier in the same uncommitted change set, so only `PROJECT_OVERVIEW.md` needed a documentation edit this pass.
- The deferred Supabase auto-sync trigger gap (modifier-only edits don't flip `ayanaon_pending_sync`, see the entry above) is NOT part of this release — it lives in the petalytix repo and remains an open follow-up, not shipped code here.
- Verification: `node --check` on both `netlify/functions/api.js` and `public/app.js`, full `npm run test:deployment` (40/40, including the 3 modifier-sanitizer/render tests). Not deployed from this session — still requires `npm run deploy:cloudflare` before the version bump takes effect in production, same as every prior release.

## Merge: reconciled two independent v2.6.3 branches (2026-08-25, later)
- `origin/cloudflare-migration` had already advanced past local `v2.6.2` with its own `v2.6.3` commit (parking/Travel Mode work, entries above dated 2026-08-16/2026-08-19) while this session independently committed its own `v2.6.3` (Modifier Groups + category icons, entries above dated 2026-08-25) — both branches picked the same next version number from the same `v2.6.2` base, and the parking commit was already pushed to GitHub before this was discovered.
- Resolution: merged both commits rather than renumbering either, since rewriting the already-pushed parking commit would require a force-push. The merged `README.md` "What's New in v2.6.3" section now lists both feature sets' bullets together; `package.json`/`package-lock.json`/`public/service-worker.js` needed no manual fix since both branches independently landed on the identical `2.6.3`/`ayanaon-static-v2.6.3` strings and merged without conflict.
- `PROJECT_OVERVIEW.md`, `CODE_RULES.md`, and `FILE_MAP.md` also auto-merged cleanly (no manual edits needed) since the two branches touched different sections of each file.

## `/toko` Modifier Groups picker: MULTI (checkbox) selection support (2026-08-29)

Cross-repo follow-up done from the **petalytix** side this session (which also updated its own `ai-memory` — check that repo's `SESSION_LEARNINGS.md`/`CODE_RULES.md` for the petalytix-side detail: `ayanaon-partner.ts`, the Products → Modifier tab's new Single/Multi toggle, and a separate receipt-summary "+"-grouping fix). This entry covers only what changed in this repo.

- **Root cause**: petalytix's Modifier tab gained a real Single/Multi selection toggle (`option_groups.selection_type/min_select/max_select`, previously always written `SINGLE/1/1`), but `ayanaon-partner.ts`'s `fetchMenuHighlights()` — the function that builds the `modifierGroups` payload pushed to this repo's `PUT /partners/ayakasir/stores` — never carried those 3 fields at all, only `{name, values: [{name, priceAdjustment}]}`. So even after an owner configured a group as Multi on the web ERP, this repo's `/toko` picker (hard-coded `input.type = 'radio'`, one selection per group) would have kept forcing single-select with zero visibility into the mismatch.
- **Fix, this repo's half**: `cleanMerchantModifierGroupValues`/`cleanMerchantModifierGroups` (`netlify/functions/api.js`) now accept `selectionType`/`minSelect`/`maxSelect` per group and **independently re-clamp them** rather than trusting the partner payload's own math — `selectionType` defaults to `'SINGLE'` (forces `minSelect=maxSelect=1`, byte-identical to pre-2026-08-29 behavior) unless the payload explicitly says `'MULTI'`; a MULTI group's `maxSelect` is clamped down to `values.length`, and `minSelect` down to never exceed the resulting `maxSelect`. Same defensive posture as this repo's existing category/price/URL sanitizers — never assume the partner side validated anything.
- `openModifierModal` renders `input.type = isMulti ? 'checkbox' : 'radio'` per value (a `(min-max)` hint appended to a MULTI group's title), and its `change` listener force-unchecks a box that would push the group's checked count past `maxSelect` — belt-and-suspenders on top of the sanitizer's server-side clamp, since the raw `data-modifiers` JSON is still just embedded in the page and technically editable client-side before this listener runs.
- New `updateModifierConfirmState()` disables `#toko-modifier-confirm` unless every rendered group's checked-input count falls within `[data-min, data-max]` (stamped as attributes on each `.toko-modifier-group` wrap at render time) — there was previously no disabled state at all, because a SINGLE group with its first radio auto-checked was always trivially "answered". That SINGLE case is unaffected (still auto-checks value 0, still always satisfies `count=1` within `[1,1]`).
- `confirmModifierSelection` switched its query from `input[type="radio"]:checked` to `input:checked` (works for both input types) and now collects however many boxes are checked per group instead of assuming exactly one. Cart-line label joins a group's multiple picks with `" + "` (e.g. `"Es Kopi Susu (Bubble + Mango Slice)"`) instead of the old one-radio-per-group `join(', ')` across groups — mirrors the exact grouping convention petalytix's `PosScreen.tsx`/`OrderView.tsx` adopted the same day for their own receipt/cart-line summaries (independently reimplemented here in vanilla JS, no shared import path). The cart-line dedupe key (`dataName|group:value;group:value...`) now sorts a MULTI group's chosen value names before joining them, so confirming the same MULTI combination in a different tap order still merges into one existing WA-cart line rather than creating a duplicate.
- **Tests**: extended `tests/merchant-visibility.test.js` — the two existing `deepEqual` assertions on sanitized `modifierGroups` shape now include the new `selectionType`/`minSelect`/`maxSelect` fields (they'd have failed otherwise, since `deepEqual` is exact-shape); added a new case covering the MULTI clamp specifically (an over-wide `minSelect:5/maxSelect:99` against 2 values clamps to `2/2`; `minSelect:0` optional-group case preserved as-is; non-numeric `minSelect:'nope'`/`maxSelect:null` degrades to `0/1` rather than throwing). Full `npm run test:deployment`: 45/45 passing (was 40/40 as of the 2026-08-25 release; +1 net test file growth from this pass plus whatever else landed between then and now — did not audit the gap, just confirmed the current total is all-green).
- **Verified**: `node --check netlify/functions/api.js` (syntax) and the full test suite above. **Not verified live** — unlike the 2026-08-25 entry above (which drove a real local `wrangler dev` + actual Supabase push + Playwright click-through), this pass had no live-push or browser verification; it's static/unit-level only. Before relying on this in production, repeat that same live-verification recipe (local `wrangler dev`, a real `pushAyanaonListing()` run via `npx tsx` from petalytix pointed at the local Worker, then Playwright or manual click-through on `/toko/<slug>`) against a tenant with an actual MULTI modifier group configured — this session did not have a live Supabase session available to do that itself.
- Also carries forward, unaffected by this change: the auto-sync trigger gap noted in the 2026-08-25 entry above (`migration_ayanaon_auto_sync.sql` still doesn't watch `option_groups`/`option_group_values`/`product_option_group_links`/`tenants.modifier_groups_enabled`) — a MULTI-group edit still won't auto-push without a manual "Perbarui Menu" or the existing workaround, same as any other modifier edit.
- **Lesson for future sessions**: check `git log origin/<branch> --oneline` (or `git fetch` + compare) before picking the next release version number, especially when other sessions/agents may be working on the same branch concurrently — this collision happened because both sides only looked at their own local `package.json` to decide the next version.

## v2.6.4 release alignment (2026-08-29)
- v2.6.4 packages the MULTI (checkbox) modifier-group selection support and the now-live modifier price adjustments documented in the entry above — both were uncommitted on top of the `v2.6.3` merge commit when this release pass started.
- Applied the lesson from the previous entry: ran `git fetch origin cloudflare-migration` and confirmed local `HEAD` matched `origin/cloudflare-migration` exactly (both at the `9f2b491` merge commit) before picking `2.6.4` as the next version, so there is no repeat of the v2.6.3 double-release collision.
- Release metadata advanced together: `package.json` + `package-lock.json` (2.6.3 → 2.6.4), README release notes, `PROJECT_OVERVIEW.md` current-version line, and the `ayanaon-static-v2.6.4` service-worker cache. `CODE_RULES.md`/`FILE_MAP.md`/`PROJECT_OVERVIEW.md`'s `merchants` collection description were already updated earlier in the same uncommitted change set, so only the version-number line needed a documentation edit this pass.
- The deferred Supabase auto-sync trigger gap and the "not verified live" caveat from the entry above are unchanged by this release-alignment pass — no live/browser verification was performed here either, only `node --check` and the automated test suite.
- Verification: `node --check netlify/functions/api.js`, full `npm run test:deployment` (45/45). Not deployed from this session — still requires `npm run deploy:cloudflare` before the version bump takes effect in production, same as every prior release.
